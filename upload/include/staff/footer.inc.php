...
<?php if (!isset($_SERVER['HTTP_X_PJAX'])) { ?>
    <div id="footer">
        ...
        <?php
            // Debug: muestra origen de la conexión a la base de datos.
            $dbHost = defined('DBHOST') ? DBHOST : __('Unknown host');
            $dbName = defined('DBNAME') ? DBNAME : __('Unknown database');
            $isLocal = stripos($dbHost, 'localhost') !== false
                || stripos($dbHost, '127.0.0.1') !== false
                || stripos($dbHost, '::1') !== false;
            $connType = $isLocal ? __('Localhost') : __('Remote');
        ?>
        <div class="muted" style="font-size:11px; margin-top:4px;">
            <?php echo sprintf(__('DB: %s @ %s (%s)'),
                Format::htmlchars($dbName),
                Format::htmlchars($dbHost),
                Format::htmlchars($connType)); ?>
        </div>
    </div>
<?php
